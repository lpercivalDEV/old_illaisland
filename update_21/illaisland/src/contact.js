...
function draw() {
  ...
  for (y = 0; y < height; y++) {
    for (x = 0; x < width; x++) {
      ...
      xoffset =
        (ripplemap[x - 1 + y * width] - ripplemap[x + 1 + y * width]) >> 5;
      yoffset =
        (ripplemap[x + (y - 1) * width] - ripplemap[x + (y + 1) * width]) >> 5;
      ...
      if (!mobile) {
        ...
        highlights.data[cur_pixel] = 218 + texture.data[new_pixel];
        highlights.data[cur_pixel + 1] = 212 + texture.data[new_pixel + 1];
        highlights.data[cur_pixel + 2] = 218;
        //Set transparency based on the xoffset. Zero-offset pixels will be transparent
        highlights.data[cur_pixel + 3] = xoffset << 3;
      }
      mapind++;
      i_++;
    }
